...
class Lexer {
  ...
  next() {
    ...
    if (c === "\n") {
      ...
      switch (this.current) {
        case " ":
          this.indent.push(this.current);
          break;
        case "	":
          throw new Error(`found tab Line: ${this.end_loc.line}, Col : ${this.end_loc.column} : indenting with tabs is not allowed`);
        default:
    ...
    const maybe_fields = () => {
      ...
      while (!(this.is_eof() || this.is_close_curly())) {
        ...
        if (!n2) {
          ...
          if (type2) {
            const id2 = { v: ["id", "sn__"], loc: { "line": 0, "column": 0 } };
            let field2 = new Field(id2, type2);
