...
class FnSig {
  ...
  params;
  ret_types;
  constructor(name, params, ret_types) {
    this.name = name;
    this.params = params;
    this.ret_types = ret_types;
  }
...
class Parser {
  ...
  run() {
    ...
    while (!this.is_eof()) {
      ...
      if (!parsed) {
        this.maybe_attrs();
        this.maybe_modifier();
        parsed = this.maybe_global_fn() || this.maybe_typedef() || this.maybe_trait() || this.maybe_impl();
      }
      if (!parsed) {
  ...
  req_alias() {
    panic$2("type alias not implemented");
  }
  req_struct(id) {
