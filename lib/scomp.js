...
function regexp(expr2) {
  return new RegExp(expr2, "u");
}
...
class TypeDef {
  ...
  fields;
  children;
  constructor(name, fields, children) {
    this.name = name;
    this.fields = fields;
...
class Parser {
  ...
  maybe_ret() {
    ...
    this.next();
    const expr2 = this.maybe_expr();
    const n = new Node("return", "expr", expr2);
    return n;
  }
  maybe_break() {
    if (!this.is_break()) {
  ...
  req_do_block() {
    const token = this.current;
    const expr2 = this.maybe_do_block();
    if (expr2) {
      return expr2;
    } else {
  ...
  maybe_lopr_prefix_postfix(expr2, postfix_op) {
    const opr = expr2.v.opr;
    const op = expr2.v.op;
    if (expr2.id === "prefix") {
      if (this.prec_uni(postfix_op) >= this.prec_uni(op)) {
        ...
        const postfix = new Uni(expr2, postfix_op);
  ...
  maybe_lopr_bin_postfix(expr2, postfix_op) {
    if (expr2.id === "bin") {
      const op = expr2.v.op;
      const ropr = expr2.v.ropr;
      const lopr = expr2.v.lopr;
  ...
  maybe_lopr_prefix_bin(expr2, bin_op) {
    const op = expr2.v.op;
    let opr = expr2.v.opr;
    if (expr2.id === "prefix") {
      if (this.prec_uni(op) >= this.prec_bin(bin_op)) {
        ...
        const ropr = this.req_ropr(expr2);
  ...
  while_op(lopr, cond) {
    ...
    while (this.is_bin_op() || this.is_postfix_uni_op()) {
      ...
      expr2 = this.req_op(expr2);
    }
    return expr2;
  }
  maybe_expr() {
    let expr2 = this.maybe_prim();
    if (expr2) {
      expr2 = this.while_op(expr2);
      return expr2;
    }
  }
  req_id() {
      ...
      if (this.is_id() && this.expect_colon()) {
        ...
        if (expr2) {
          ...
          if (this.is_comma()) {
            ...
            this.req_close_paren();
            const afn = this.maybe_afn(expr2);
            if (afn) {
              return afn;
            } else {
              expr2.grouped = true;
  ...
  req_named_tuple() {
    ...
    while (!(this.is_eof() || this.is_close_paren())) {
      ...
      this.req_colon();
      let expr2 = this.req_expr();
      named_tuple.push([name, expr2]);
      this.optional_comma();
    }
    this.req_close_paren();
  ...
  maybe_literal() {
    let expr2 = this.maybe_primitivel();
    if (!expr2) {
      expr2 = this.maybe_list();
    }
    if (!expr2) {
      expr2 = this.maybe_tuple();
    }
    return expr2;
  }
  maybe_primitivel() {
    let expr2 = this.maybe_bool();
    if (!expr2) {
      expr2 = this.maybe_char();
    }
    if (!expr2) {
      expr2 = this.maybe_str();
    }
    if (!expr2) {
      expr2 = this.maybe_int();
    }
    if (!expr2) {
      expr2 = this.maybe_float();
    }
    return expr2;
  }
  maybe_bool() {
  ...
  maybe_when_arm() {
    ...
    this.req_thin_arrow();
    const expr2 = this.req_expr();
    const arm = new WhenArm(pats, expr2);
  ...
  maybe_when() {
    ...
    this.maybe_comma();
    this.req_close_curly();
    const _when = new When(expr2, arms);
  ...
  maybe_typedef() {
    ...
    if (children) {
      const id2 = { v: ["id", "children"], loc: { "line": 0, "column": 0 } };
      const t = { "id": "[", "t": "t", "v": { "t": { "id": "t", "t": "t", "v": { "t": { "v": ["id", "any"], "loc": { "line": 0, "column": 0 } } } } } };
      const field = new Node("field", "expr", [id2, t]);
      fields.push(field);
    }
...
`);
  ...
  write_typedef(_typedef) {
    ...
    this.indent_level += 1;
    if (_typedef.fields) {
      this.write_fields(_typedef.fields);
    }
...
`);
  ...
  write_ref(expr2) {
    const _ref = expr2.v.v[1];
    this.append(_ref);
  }
  write_str(expr2) {
    const str = expr2.v.v[1];
    const symbol2 = str.indexOf("${") === -1 ? '"' : "`";
    this.append(symbol2 + str + symbol2);
  }
  write_str_id(expr2) {
    this.append(symbol + expr2.v.v[1] + symbol);
  }
  is_call(expr2) {
    return expr2.v.id === "bin" && expr2.v.v.op.v === "(";
  }
  write_iret(expr2) {
    if (!(contains(["when", "while", "if", "for", "return"], expr2.v.node) || expr2.v.t === "()" || this.is_call(expr2) || expr2.semicolon)) {
      this.append("return ");
    }
    if (this.is_call(expr2)) {
      this.append("const temp_seen_var = ");
      this.write_expr(expr2.v);
      this.append("\n");
  ...
  write_list(expr2) {
    this.append("[");
    let i = 0;
    const length = expr2.v.length;
    while (i < length) {
      this.write_expr(expr2.v[i]);
      if (i < expr2.v.length - 1) {
        this.append(", ");
      }
      i += 1;
    }
  ...
  write_named_tuple(expr2) {
    this.append("{");
    expr2.v.forEach((pair, i) => {
      const k = pair[0].v[1];
      const v2 = pair[1];
      this.append(k);
      this.append(": ");
  ...
  write_when(expr2) {
    this.appendi("switch(");
    this.write_expr(expr2.v.expr);
    this.append(") {\n");
    this.indent_level += 1;
    let i = 0;
    while (i < expr2.v.arms.length) {
      const arm = expr2.v.arms[i];
  ...
  write_prefix_uni(expr2) {
    const op = expr2.v.op.v;
    switch (op) {
      case ".":
        {
          if (expr2.v.opr.v.v[1] === "none") {
            this.append("null");
            return;
          } else {
            panic("enum variants are not supported, found : (." + expr2.v.opr.v.v[1] + ")");
          }
        }
        break;
      case "not":
    ...
    while (stack.length > 0) {
      ...
      switch (expr2.id) {
        ...
        case "named_tuple":
          ...
          break;
        default:
          throw new Error("syntax error |> :" + to_str(expr2));
      }
    }
  }
  write_runtime_fn() {
  ...
  write_call(expr2) {
    const runtime_impl = this.runtime && this.runtime.get_fn(expr2);
    if (runtime_impl) {
    ...
    this.to_en_id(expr2.v[0].v);
    if (expr2.v[0].v.v[1] === "html") {
      const page = this.html_gen.en.write_html(expr2, "");
      this.append(` (() => \`${page}\`)() `);
      return;
    } else if (expr2.v[0].v.v[1] === "صفحة_الشبكة") {
      const page = this.html_gen.ar.write_ar_html(expr2, "");
      this.append(`(() => \`${page}\`)()`);
      return;
    } else if (this.symtab.structs.includes(expr2.v[0].v.v[1])) {
      this.append("new ");
    }
    this.write_expr(expr2.v[0]);
    this.append("(");
    const args = expr2.v[1];
    if (args) {
      args.forEach((arg, i) => {
        this.write_expr(arg);
        if (i < args.length - 1) {
  ...
  write_bin(expr2) {
    ...
    switch (op) {
      case "[":
        {
          this.write_expr(expr2.v.lopr);
          this.append("[");
          this.write_expr(expr2.v.ropr);
          this.append("]");
        }
        break;
      case "=":
      ...
      case "|>": {
        let stack = [];
        let lhs = expr2.v.lopr;
        let rhs = expr2.v.ropr;
        while (true) {
          stack.push(lhs);
      ...
      case "||>":
        throw new Error(" ||> : WIP , " + to_str(expr2));
      case ":>":
        throw new Error(" :> : WIP , " + to_str(expr2));
      case "==":
      case "!=":
      ...
      case ".":
        ...
        break;
      default:
        panic("cannot write binary operation: " + to_str(expr2));
        break;
    }
  }
  write_afn(expr2) {
    this.push();
    this.write_params(expr2.v.params);
    this.append("=>");
    this.write_body(expr2.v.body);
    this.pop();
  }
  write_expr(expr2) {
    if (expr2.grouped) {
    ...
    switch (expr2.id) {
      ...
      case ";":
        break;
      case "ref":
        this.write_ref(expr2);
        break;
      case "bool":
        this.append(expr2.v.v[1]);
        break;
      case "int":
      case "float":
        this.append(to_maghrib_num(expr2.v[0].v[1]));
        break;
      case "char":
        this.append("'" + expr2.v.v[1] + "'");
        break;
      case "str":
        this.write_str(expr2);
        break;
      case "return":
        this.write_ret(expr2);
        break;
      case "iret":
        this.write_iret(expr2);
        break;
      case "[":
        this.write_list(expr2);
        break;
      case "{":
        this.write_structl(expr2);
        break;
      case "args":
        this.write_args(expr2);
        break;
      case "named_arg":
        this.write_named_arg(expr2);
        break;
      case "tuple":
        this.write_tuple(expr2);
        break;
      case "named_tuple":
        this.write_named_tuple(expr2);
        break;
      case "when":
        this.write_when(expr2);
        break;
      case "do_block":
        this.write_do_block(expr2);
        break;
      case "block":
        this.write_block(expr2);
        break;
      case "prefix":
        this.write_prefix_uni(expr2);
        break;
      case "call":
        this.write_call(expr2);
        break;
      case "bin":
        this.write_bin(expr2);
        break;
      case "afn":
        this.write_afn(expr2);
        break;
      default:
        panic("cannot write expr: " + to_str(expr2));
    }
    if (expr2.grouped) {
      this.append(")");
    }
