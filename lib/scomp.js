...
class Parser {
  ...
  maybe_stmt() {
    ...
    return this.maybe_break() || this.maybe_val() || this.maybe_let() || this.maybe_expr() || this.maybe_semicolon() || this.maybe_for();
  }
  req_stmts() {
  ...
  implicit_return(_stmts) {
    ...
    if (contains$2(["when", "while", "if", "for", "for_inf", "for_cond", "for_in", "return", "let", "var", "const"], last.id) || last.id === "bin" && last.v.op.v === ":=") {
      return;
    }
    if (last.id === ";") {
      return;
