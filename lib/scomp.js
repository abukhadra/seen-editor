      ...
      if (this.indent != null) {
        switch (this.current) {
          case " ":
            ...
            break;
          case "	":
            throw new Error(`Tabs are not allowed! replace tab at (Line: ${this.start_loc.line}, Col : ${this.start_loc.column}) with spaces`);
          default:
    ...
    while (!this.expect_eof()) {
      ...
      switch (c) {
        ...
        case "%":
          ...
          break;
        case ".":
          this.maghrib_float() || this.dots() || this.dot();
          break;
        case ",":
          this.add_token(",");
          break;
...
class Method {
  ...
  instance_type;
  fn;
  constructor(trait, instance_type, fn) {
    this.trait = trait;
...
class Fn {
  ...
  params;
  body;
  constructor(attrs, typesig, modifier, name, params, body) {
    this.attrs = attrs;
...
class Parser {
  ...
  run() {
    ...
    while (!this.is_eof()) {
      ...
      if (!parsed) {
        ...
        parsed = this.maybe_global_method(attrs, modifier, typesig) || this.maybe_global_fn(attrs, typesig, modifier);
      }
      if (!parsed) {
        parsed = this.maybe_typedef() || this.maybe_trait();
      }
      if (!parsed) {
  ...
  maybe_stmt() {
    ...
    return this.maybe_break() || this.maybe_val() || this.maybe_let() || this.maybe_expr() || this.maybe_semicolon() || this.maybe_for() || this.maybe_while();
  }
  req_stmts() {
  ...
  stmts() {
    let _stmts = [];
    let stmt;
    if (this.get_indent() <= this.current_indent + 1) {
      return [];
    }
    this.current_indent = this.get_indent();
    while (true) {
      if (this.get_indent() < this.current_indent) {
        break;
      }
  ...
  maybe_trait() {
    panic$2("traits not implemented!");
  }
  maybe_val() {
    if (this.is_indent()) {
      return;
    }
    if (!(this.is_id() && this.expect_val_asgmt() || this.expect_colon())) {
      return;
    }
  ...
  maybe_while() {
    ...
    let expr;
    let body;
    if (!this.is_colon() || !this.is_open_curly()) {
      expr = this.req_expr();
    }
    if (this.is_colon()) {
      body = this.req_body();
  ...
  req_anonymous_method() {
    ...
    const no_indent = () => {
      ...
      let amethod2 = new AnonymousMethod(stmts);
      this.req_close_curly();
      return amethod2;
    };
    const with_indent = () => {
  ...
  maybe_global_method(attrs, modifier, typesig) {
    if (this.is_indent() && !this.is_open_paren()) {
      return;
    }
  ...
  maybe_type() {
    return this.maybe_simple_type() || this.maybe_list_type() || this.maybe_infer_type();
  }
  req_type() {
  ...
  implicit_return(_stmts) {
    ...
    if (contains$2(["when", "while", "if", "for", "while", "return", "let"], last.id) || last.id === "bin" && last.v.op.v === ":=") {
      return;
    }
    if (last.id === ";") {
      return;
...
`);
  ...
  write_trait(_trait) {
    panic("trait is not implemented yet.");
  }
  write_trait_impl(_trait_impl) {
    panic("trait implementation is not implemented yet.");
  }
  write_method(_method) {
    panic("method is not implemented yet.");
  }
  write_const(_const) {
    this.appendi("const ");
...
`);
  ...
  write_iret(expr) {
    if (!(contains(["when", "while", "if", "for", "for_inf", "for_cond", "for_in", "return"], expr.v.node) || expr.v.t === "()" || expr.v.t === "void" || expr.v.t === "" || this.is_call(expr) || expr.semicolon)) {
      this.append("return ");
    }
  ...
  write_call(expr) {
    if (expr.v.trailing.length > 0) {
      panic("trailing closures is not implemented yet");
    }
    ...
    if (expr.v.id.v.v[1] === "html") {
      ...
      this.append(`(() => \`${page}\`)()`);
      return;
    } else if (this.symtab.structs.includes(expr.v.id.v.v[1])) {
      this.append("new ");
    }
