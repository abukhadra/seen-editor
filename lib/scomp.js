...
class Parser {
  ...
  maybe_typedef() {
    ...
    if (children) {
      const id2 = { v: ["id", "__children"], loc: { "line": 0, "column": 0 } };
...
class JSGen {
  ...
  init_with_HTML(html_gen, ast, symtab, main_args, opts) {
    this.init(ast, symtab, html_gen, main_args, opts);
    this.html_gen = html_gen;
  }
  init_with_runtime(runtime, ast, symtab, main_args, opts) {
...
`);
  ...
  write_iret(expr) {
    if (!(contains(["when", "while", "if", "for", "return"], expr.v.node) || expr.v.t === "()" || this.is_call(expr) || expr.semicolon)) {
      this.append("return ");
    }
